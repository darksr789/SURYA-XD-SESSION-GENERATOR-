    ...
    try {
      ...
      SuryaPairWeb.ev.on("connection.update", async (s) => {
        ...
        if (connection === "open") {
          try {
            await delay(10000);
            const sessionSurya = fs.readFileSync("./session/creds.json");
